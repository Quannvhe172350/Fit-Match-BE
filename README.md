...
- Không cấu hình `SMS_PROVIDER` → OTP SMS chỉ được log ra console (`[SMS-STUB]`). Gửi thật: `SMS_PROVIDER=speedsms` (speedsms.vn — khuyến nghị cho demo VN, tặng credit test), `twilio` (trial nhưng VN bị hạn chế verify số nhận), hoặc `esms` (esms.vn, production brandname). Xem `.env.example`.
